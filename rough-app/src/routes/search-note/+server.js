...
export async function POST({ request }) {
  try {
    ...
    if (!apiKey || !text) {
      return json({ error: "Missing apiKey or text" }, { status: 400 })
    }
    ...
    if (text.startsWith("User:")) {
      const exactUserMatches = matchesWithMetadata.filter(
        (match) => match.metadata.full_text && match.metadata.full_text.includes(text),
      )
    ...
    try {
      const response2 = await pc.inference.rerank(rerankingModel, query, documents, rerankOptions)
      ...
      console.warn("Reranking failed, using original matches:", rerankError)
      // Keep the original matches from the vector search
    }
